 ...
 public:
  ...
  bool SolveEmProblem() {
    ...
    while (T+block_size < state_vec_.size()) {
      ...
      for (size_t i=0; i<landmark_vec_.size(); ++i) {
        opt_problem.AddParameterBlock(landmark_vec_.at(i)->parameters(), 3);
      }

      for (size_t t=T; t < T+block_size; ++t) {
        opt_problem.AddParameterBlock(state_vec_.at(t)->GetRotationBlock()->parameters(), 4);  // bug with quat_parameterization_ptr_
        opt_problem.AddParameterBlock(state_vec_.at(t)->GetVelocityBlock()->parameters(), 3);
        opt_problem.AddParameterBlock(state_vec_.at(t)->GetPositionBlock()->parameters(), 3);

        opt_problem.SetParameterBlockConstant(state_vec_.at(t)->GetRotationBlock()->parameters());
        opt_problem.SetParameterBlockConstant(state_vec_.at(t)->GetVelocityBlock()->parameters());
        opt_problem.SetParameterBlockConstant(state_vec_.at(t)->GetPositionBlock()->parameters());


        for (size_t j=0; j<observation_vec_.at(t-1).size(); ++j) {
          size_t landmark_idx = observation_vec_.at(t-1).at(j)->landmark_id_-1;  

          ceres::CostFunction* cost_function = new ReprojectionError(observation_vec_.at(t-1).at(j)->feature_pos_,
                                                                     T_bc_,
                                                                     fu_, fv_,
                                                                     cu_, cv_,
