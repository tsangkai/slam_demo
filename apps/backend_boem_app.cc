  ...
  bool SolveBoemProblem() {
    ...
    while (!reach_end) {
      ...
      for (size_t i=T; i <T+block_size; ++i) {
        opt_problem.AddParameterBlock(state_vec_.at(i+1)->GetRotationBlock()->parameters(), 4);  // bug with quat_parameterization_ptr_
        opt_problem.AddParameterBlock(state_vec_.at(i+1)->GetVelocityBlock()->parameters(), 3);
        opt_problem.AddParameterBlock(state_vec_.at(i+1)->GetPositionBlock()->parameters(), 3);

        opt_problem.SetParameterBlockConstant(state_vec_.at(i+1)->GetRotationBlock()->parameters());
        opt_problem.SetParameterBlockConstant(state_vec_.at(i+1)->GetVelocityBlock()->parameters());
        opt_problem.SetParameterBlockConstant(state_vec_.at(i+1)->GetPositionBlock()->parameters());
