 ...
 public:
  ...
  bool SolveBoemProblem() {
    ...
    while (!reach_end) {
      ...
      for (size_t i=T; i <T+block_size; ++i) {
        opt_problem.AddParameterBlock(state_para_vec_.at(i+1)->GetRotationBlock()->parameters(), 4, quat_parameterization_ptr_);
...
int main(int argc, char **argv) {
  ...
  slam_problem.OutputResult("result/sim_easy/boem.csv");
